...
//		int currentgen = ;
	...
	for (int gen = start_gen + 1; gen < GENS; gen++) {
		//if (((gen % 100000 == 0)) && (gen > 0))
		//if (gen == 0 || gen==10 ||gen == 100 || gen ==1000 || gen==10000 ||gen ==100000){
		/*if (gen == 0 || gen == 10 || gen == 100 || gen == 1000 || gen == 10000 || gen == 100000 || gen == 1000000){
		...
		for (int i = 0; i< selection_strength; i++) {
			...
			Chromosome g1 = pop.at(parent1).ForRecombination(mutRate);
			Chromosome g2 = pop.at(parent2).ForRecombination(mutRate);
